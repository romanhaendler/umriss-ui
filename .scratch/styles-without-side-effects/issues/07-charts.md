...
**Review follow-up** (see the spec's Comments): the benchmark's and the `sizes` example's controls carry type and ring in `demo.css` instead of being tolerated; three wrong token names in `demo.css` (`--u-font-size-*`) are corrected.

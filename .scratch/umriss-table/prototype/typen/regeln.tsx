  ...
  return (
    <>
      <Table>
        ...
        {/* @ts-expect-error R3 Zahlenmethode auf Text */}
        <Column value="nummer" label="Nummer" children={(nummer) => nummer.toFixed(2)} />
        {/* @ts-expect-error R3 berechneter Text, Zahlenmethode */}
        ...
        {/* @ts-expect-error R4 Array ohne children */}
        <Column value="tags" label="Tags" />
        <Column id="kundeObj" value={(a) => a.kunde} label="Kunde">
          {(kunde) => kunde.name}
        </Column>
        {/* @ts-expect-error R4 berechnetes Objekt ohne children */}
        ...
        <Column {...zweiFelder} />
        {/* @ts-expect-error R8 Preset auf Zeilen, deren menge Text ist */}
        <m.Column {...textMenge} />
        {/* @ts-expect-error R8 Preset auf Zeilen ohne menge */}
        <m.Column {...menge} />

        {/* R9 Ein Renderer, der eine andere Zeilenart erwartet. */}
        {/* @ts-expect-error R9 Zeile der falschen Art */}
        ...
        <MengenSpalte of={t} />
        {/* @ts-expect-error R10 Huelle fuer eine andere Zeilenart */}
        <MengenSpalte of={m} />
...
export function GenerischeMenge<Z extends { menge: number }>({ of }: { of: Tabelle<Z> }) {
  const { Column } = of;
  // @ts-expect-error R10 generische Huelle
  return <Column value="menge" label="Menge" footer="sum" />;
}
...
export function Aktionen() {
  ...
  return (
    <>
      ...
      {/* @ts-expect-error R13 Zeilenaktion behandelt eine Liste */}
      <Action onSelect={(liste) => liste.map((a) => a.nummer)}>Öffnen</Action>
      {/* @ts-expect-error R13 Sammelaktion behandelt eine Zeile */}
...
export function Urteile() {
  ...
  return (
    <>
      ...
      {/* @ts-expect-error R14 Urteil ueber Text */}

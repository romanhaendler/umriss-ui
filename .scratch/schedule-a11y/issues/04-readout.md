...
Wording in core, English and German: `scheduleRoleDescription` ("schedule" / "Belegungsplan" - not "Plantafel", which CONTEXT.md avoids), `scheduleSummary`, `scheduleKeyHelp`. German names the subtask *Teilaufgabe* (CONTEXT.md avoids *Arbeitsgang*), now written into the glossary. The readout reuses the tooltip's entries. Seen on the way, not changed: the existing `hoursShort` says "1 hrs".

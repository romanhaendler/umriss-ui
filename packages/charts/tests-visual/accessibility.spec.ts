/* Accessibility check of a sample of pages of the demo of @umriss-ui/charts,
   light and dark, against WCAG 2.1 AA.

   The standards, the tolerated colour pairs and the reading of the findings
   come from the shell (`@umriss-ui/demo/checks/accessibility.ts`): they are the
   pairs the suite of @umriss-ui/core tolerates, with the same written reasons,
   and not one has been added for the charts. Its two guards - every entry
   carries a reason, an open one points at a follow-up - stand there, where the
...
test("a page with every code block open is accessible", async ({ page }, testInfo) => {
  ...
  await expect(page.locator('[data-block="axis"] .codeBlock').first()).toBeVisible();

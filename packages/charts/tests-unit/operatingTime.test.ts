...
/** Two early shifts on two days, in hours since a zero point:
    ...
    Operating time by hand:
      6 o'clock  → 0 h        14 o'clock → 8 h
      30 o'clock → 8 h        38 o'clock → 16 h  (that is the whole operating time)
    Removed is everything before, everything after, and the 16 hours in between. */
const shifts = operatingCalendar([
  { from: 6 * HOUR, to: 14 * HOUR },
  { from: 30 * HOUR, to: 38 * HOUR },
]);

describe("operatingCalendar - the list of intervals in which time counts", () => {
  it("names the whole operating time", () => {
    // By hand: twice eight hours.
    expect(shifts.total).toBe(16 * HOUR);
  });

  it("sorts unsorted intervals", () => {
    const k = operatingCalendar([
      { from: 30 * HOUR, to: 38 * HOUR },
      { from: 6 * HOUR, to: 14 * HOUR },
    ]);
...
describe("toOperatingTime - wall clock time inwards", () => {
  it("maps the beginning of an interval onto its accumulated time", () => {
    expect(toOperatingTime(6 * HOUR, shifts)).toBe(0);
    expect(toOperatingTime(30 * HOUR, shifts)).toBe(8 * HOUR);
  });
  ...
  it("maps the end of an interval from the other side onto the same value", () => {
    ...
    // operating time - that is exactly the point of the axis.
    expect(toOperatingTime(14 * HOUR, shifts)).toBe(8 * HOUR);
    expect(toOperatingTime(30 * HOUR, shifts)).toBe(8 * HOUR);
    expect(toOperatingTime(38 * HOUR, shifts)).toBe(16 * HOUR);
  });

  it("maps points in the interior linearly", () => {
    // By hand: 10 o'clock is four hours after the start of the shift.
    expect(toOperatingTime(10 * HOUR, shifts)).toBe(4 * HOUR);
    // 34 o'clock is eight accumulated plus four hours.
    expect(toOperatingTime(34 * HOUR, shifts)).toBe(12 * HOUR);
  });
  ...
  it("turns a point in a removed interval into a gap", () => {
    ...
    expect(toOperatingTime(20 * HOUR, shifts)).toBeNaN();
    expect(toOperatingTime(14 * HOUR + 1, shifts)).toBeNaN();
    expect(toOperatingTime(30 * HOUR - 1, shifts)).toBeNaN();
  });

  it("turns points before and after the calendar into a gap too", () => {
    expect(toOperatingTime(0, shifts)).toBeNaN();
    expect(toOperatingTime(48 * HOUR, shifts)).toBeNaN();
  });

  it("stays monotonically non-decreasing across the removed time", () => {
    const before = toOperatingTime(13 * HOUR, shifts);
    const after = toOperatingTime(31 * HOUR, shifts);
  ...
  it("returns a gap as a gap", () => {
    expect(toOperatingTime(Number.NaN, shifts)).toBeNaN();
  });
});

describe("toWallClock - operating time back", () => {
  ...
  it("chooses the beginning of the next interval at a seam", () => {
    // 8 h of operating time are two wall clock times: 14 o'clock on the first day
  ...
  it("has no wall clock time outside the operating time", () => {
  ...
  it("comes back unchanged for points inside the operating intervals", () => {
    for (const wallClock of [6 * HOUR, 7.5 * HOUR, 13 * HOUR, 30 * HOUR, 37 * HOUR]) {
      expect(toWallClock(toOperatingTime(wallClock, shifts), shifts)).toBe(wallClock);
    }
  });
});

describe("the empty calendar", () => {
  const empty = operatingCalendar([]);

  it("behaves in both directions like no calendar at all", () => {
    // Tested against the unmapped values, not against another run.
    expect(toOperatingTime(20 * HOUR, empty)).toBe(20 * HOUR);
    expect(toOperatingTime(-5, empty)).toBe(-5);
  ...
  it("leaves every tick where it is", () => {
    const ticks = operatingTimeTicks(empty, 0, 4 * HOUR, 2 * HOUR);
    expect(ticks).toEqual([
      { wallClock: 0, operatingTime: 0 },
      { wallClock: 2 * HOUR, operatingTime: 2 * HOUR },
      { wallClock: 4 * HOUR, operatingTime: 4 * HOUR },
    ]);
  });
});
...
describe("operatingTimeTicks - candidates in wall clock time, then mapped", () => {
  it("throws candidates in removed intervals away and maps the rest", () => {
    ...
    const ticks = operatingTimeTicks(shifts, 0, 48 * HOUR, 4 * HOUR);
    expect(ticks.map((t) => t.wallClock / HOUR)).toEqual([8, 12, 32, 36]);
    expect(ticks.map((t) => t.operatingTime / HOUR)).toEqual([2, 6, 10, 14]);
  });
  ...
  it("keeps the edges of the shifts and leaves the seam standing only once", () => {
    ...
    // inside the shifts. 14 o'clock and 30 o'clock both fall on 8 h of operating
    // time; two labels on the same pixel would be one too many, so the earlier one
    // stays.
    const ticks = operatingTimeTicks(shifts, 0, 48 * HOUR, 2 * HOUR);
    expect(ticks.map((t) => t.wallClock / HOUR)).toEqual([6, 8, 10, 12, 14, 32, 34, 36, 38]);
    expect(ticks.map((t) => t.operatingTime / HOUR)).toEqual([0, 2, 4, 6, 8, 10, 12, 14, 16]);
  });
  ...
  it("places the candidates on multiples of the step, shifted by the offset", () => {
    ...
    const ticks = operatingTimeTicks(shifts, 0, 48 * HOUR, 12 * HOUR, 2 * HOUR);
    expect(ticks.map((t) => t.wallClock / HOUR)).toEqual([14, 38]);
  });

  it("yields nothing for a reversed or empty extent", () => {
    expect(operatingTimeTicks(shifts, 10 * HOUR, 8 * HOUR, HOUR)).toEqual([]);
    expect(operatingTimeTicks(shifts, 0, 48 * HOUR, 0)).toEqual([]);
  });
});
...
describe("removedIntervals - what is to be marked on the axis", () => {
  it("names every removed span with its place in operating time", () => {
    // By hand: removed is 14 o'clock to 30 o'clock, and in operating time that
    // collapses onto the point 8 h.
    expect(removedIntervals(shifts)).toEqual([
      { from: 14 * HOUR, to: 30 * HOUR, operatingTime: 8 * HOUR },
    ]);
  });
  ...
  it("names only the gaps between intervals, not those before and after", () => {
    ...
    expect(removedIntervals(k)).toEqual([
      { from: 10, to: 20, operatingTime: 10 },
      { from: 30, to: 40, operatingTime: 20 },
    ]);
  });
});
...
describe("the raw interval list as an input", () => {
  ...
  it("computes the same from the unsorted list as from the calendar", () => {
    expect(toOperatingTime(34 * HOUR, list)).toBe(12 * HOUR);
    expect(toOperatingTime(20 * HOUR, list)).toBeNaN();
  ...
  it("lets an empty list act like no calendar at all", () => {
    expect(toOperatingTime(20 * HOUR, [])).toBe(20 * HOUR);
    expect(toWallClock(20 * HOUR, [])).toBe(20 * HOUR);
  });
});

describe("breaks - the break points within the extent of the axis", () => {
  it("yields the place in operating time at which time was removed", () => {
    // By hand: the only gap falls on 8 h of operating time.
...
describe("operatingTicks - ticks for an extent in operating time", () => {
  it("calculates the extent back, generates in wall clock time and maps", () => {
    // Extent 0…16 h of operating time, eight ticks: step 2 h. In wall clock time
    // that is 6 o'clock to 38 o'clock; by hand 6, 8, 10, 12, 14 and 32, 34, 36, 38
    // survive - 30 o'clock coincides with 14 o'clock.
    expect(operatingTicks(shifts, 0, 16 * HOUR, 8).map((v) => v / HOUR)).toEqual([
      0, 2, 4, 6, 8, 10, 12, 14, 16,
    ]);
  });

  it("copes with an extent widened to nice boundaries", () => {
    // A "nice" domain reaches beyond the operating time that exists; that must
    // generate no ticks out of nowhere.
    const values = operatingTicks(shifts, -2 * HOUR, 20 * HOUR, 8);

/* Smoke test of the demo of @umriss-ui/calculation: it mounts in jsdom, every
...
describe("Demo smoke test", () => {
  it("mounts the shell without an error", async () => {
    ...
    expect(host.textContent).toContain("Umriss Calculation");
    await unmount();
  });
...
describe("The examples as a set", () => {
  ...
  it("name the package by name and carry no title in the source", () => {
    for (const example of EXAMPLES) {
      ...
      expect(example.source, where).toContain('from "@umriss-ui/calculation"');
    }
  });

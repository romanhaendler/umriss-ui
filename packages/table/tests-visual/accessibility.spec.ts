/* Accessibility check of a sample of pages of the demo of @umriss-ui/table, light
...
test("a page with every code block open is accessible", async ({ page }, testInfo) => {
  await open(page, "rowactions");
  await allWithCode(page);
  await expect(page.locator('[data-block="rowactions"] .codeBlock').first()).toBeVisible();
  const result = await new AxeBuilder({ page }).include('[data-block="rowactions"]').withTags(STANDARDS).analyze();

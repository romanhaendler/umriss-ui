...
import type { Subtask, Task, Dependency, DependencyRoute } from "../../../src";

export const title = "The shape of a dependency's line";

/* `route` gives a dependency's line its shape, and nothing else:
   ...
   It is the schedule's option, and a single dependency may say otherwise for
   itself. Where its line touches the bars is `attach`, and whether its ends
   carry a dot is `ends` - both on the Dependencies page, each its own thing.

   None of the three touches a finding. Whether a dependency is violated
...
function Variant({ route }: { route: DependencyRoute }) {
  return (
    <Stack gap={1}>

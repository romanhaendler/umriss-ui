...
import type { PlaceIntent, Subtask, Transport } from "../src/model";
...
const TRANSPORT: Transport = { id: "t", from: "a-1", to: "a-2", duration: 5 * MIN };
...
describe("a place intent", () => {
  ...
  it("carries no teardown where none was declared", () => {
    expect("teardown" in subtaskFromPlace(PLACE, "x")).toBe(false);
  });

  it("pushes nothing: the subtask it asks for does not exist yet", () => {
    expect(ripple([EXISTING], [TRANSPORT], PLACE)).toEqual([]);
  });
});

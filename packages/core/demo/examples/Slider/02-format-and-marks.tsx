...
/* `format` writes the value as text - once for the readout and once for the
   ...
   marks without words and no readout, and says its value in the hint. */
  ...
  return (
    <Grid minItemWidth="260px" gap={6}>
      ...
      <FormField label="Fill quantity" hint="500 ml - locked while the batch runs.">
        <Slider min={450} max={550} defaultValue={500} format={(v) => `${v} ml`} showValue={false} disabled marks={[450, 500, 550]} />
      </FormField>
    </Grid>
  );
}

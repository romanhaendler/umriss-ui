...
| `Popover` | the one closable, anchored surface underneath Menu, Combobox and the pickers: portal (nearest `<dialog>`, then the portal target, then the body), position with clamping and flipping, outside click, Escape with focus returned |

...
- **More languages than English and German, and right-to-left.** Two wordings
  ...
  for a library whose readers work in German and English that is a second
...
- **Rich text, carousel, and the rest of a marketing site's parts.** An editor
  ...
  all but one value at a time - the opposite of a data-dense screen, which
  shows everything at once. *Instead:* a dedicated editor; for a carousel, a list.
...
- **Pivot.** A pivot turns rows into columns chosen at runtime, and with it the
  ...
  *Instead:* compute the pivoted rows and declare their columns; grouping with
  aggregates (ADR-0029) covers "the same measure per team and week".
...
- **Pie, donut, radar, candlestick.** The set of kinds is closed - there is no
  ...
  a data-dense screen asks: a share is read better from sorted bars (`pareto()`), a
...
- **A log scale and a category scale.** Every scale is affine, and the drawing
  loop and the working-time axis rely on it (ADR-0001); categories are numeric
...
- **WebGL.** Downsampling keeps a week of seconds under 25 ms on Canvas 2D
  ...
  every kind for figures no data-dense screen needs. *Instead:* a WebGL specialist
  for tens of millions of points.
- **Smoothing, animation, export, horizontal bars.** A smoothed curve invents
  values nobody ever measured, an animated one is harder to read, a canvas
...
- **Dependency types and a critical path.** Finish-to-start, start-to-start and
  ...
  it computes dependencies and findings, never a plan. A Gantt engine is the
...
- A request for any of these is answered with this ADR, not with a discussion.
  It is not a wall: a caller who brings a question of a data-dense application
  one of them answers reopens it, and the ADR is superseded, not edited.
- umriss will look narrower than a general library in every comparison table.
  ...
  test suite, a design pass and a maintenance share that the components for
  data-dense applications would otherwise get.
